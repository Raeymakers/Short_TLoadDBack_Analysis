...
PVT <- group_by(data, Test, Day, Condition, ID) %>%
  summarise(
    count = n(),
...
# }
# data = subset(data, select = -c(Local.Date, Participant.Private.ID, Task.Name, Question.Key)) 
# VAS<-data
...
# VAS$Time <- as.POSIXct(VAS$Time, format=  "%Y-%m-%d %H:%M", tz = "UTC")
