...
pl
qqPlot(PVT$MeanRT) #still not the best but already much better
...
ggboxplot(PVT, x = "Condition", y = "MeanRT", 
          color = "Condition", palette = c("#00AFBB", "#E7B800"),
          ylab = "MeanRT", xlab = "Condition")

#normality
PVT %>%
  group_by(Day) %>%
  shapiro_test(MeanRT) # normal distribution
ggqqplot(PVT, 'MeanRT', facet.by='Condition')
...
#visualize
two_w <- lmer('MeanRT ~ Day * Condition + (1|ID)', data= PVT)
emmeans1 = emmeans(two_w, pairwise ~ Day * Condition , adjust ="fdr", type = "response")
emmean_dataframe <- summary(emmeans1)$emmeans
emmeans1
Anova(two_w, type='III')
 
pl<- ggplot(PVT, aes(x= Day, y = MeanRT))+
...
sum <- sum_1 (EXP, 'Condition', 'MeanResp')
sum
## plot 
