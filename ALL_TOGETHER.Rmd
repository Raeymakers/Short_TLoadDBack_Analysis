...
# Install packages
list.of.packages <- c('tidyverse', 'effects', 'emmeans', 'data.table', 'dplyr', 'lme4', 'ggplot2', 'MuMIn', 'effsize', 'reshape2','cowplot', 'car', 'ggpubr', 'rstatix') # All relevant packages
...
res.aov2 <- aov(Mean ~ Condition * Day * Time, data= PVT)
...
#########################################C
 ...
 VAS <- VAS[!(VAS$ID==49),] #remove ID 49
...
#HCL
 ...
 #LCL
 VAS_f <- df$VASf[df$Time==2 & df$Condition=='LCL' & df$Day==2] - df$VASf[df$Time==1 & df$Condition=='LCL']
 ACC<- df$Acc[df$Time==2 & df$Condition=='LCL' & df$Day==2] - df$Acc[df$Time==1 & df$Condition=='LCL']
 DELTALCL_2<- corrplot(VAS_f, ACC, 'VASf delta', 'Performance delta') +ggtitle('delta correlations LCL Day 2')
 
DELTACORR <- ggarrange(DELTAHCL_1, DELTALCL_1, DELTAHCL_1, DELTALCL_2 + rremove("x.text"),
           labels = c("A", "B", "C", "D"),
           ncol = 2, nrow = 2)
